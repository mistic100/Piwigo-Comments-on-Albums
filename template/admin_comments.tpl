...
<div class="titrePage">
  <h2>{'Pending Comments'|@translate} [{'Albums'|@translate}]</h2>
</div>
...
<table>
  {foreach from=$comments item=comment name=comment}
    <tr valign="top" class="{if $smarty.foreach.comment.index is odd}row2{else}row1{/if}">
      ...
      <td>
        <div class="comment">
          <a class="illustration" href="{$comment.CAT_URL}"><img src="{$comment.TN_SRC}"><br/>{$comment.CAT_NAME}</a>
  ...
  <p class="bottomButtons">
    <input class="submit" type="submit" name="validate_albums" value="{'Validate'|@translate}">
    <input class="submit" type="submit" name="reject_albums" value="{'Reject'|@translate}">
  </p>
